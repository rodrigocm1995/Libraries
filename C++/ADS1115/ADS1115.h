...
class ADS1115{
  public:
    ADS1115(); // Constructor. It must be defined with the same name as the class

    void  init(uint8_t devAddress, TwoWire *wire = &Wire);
    void  init(uint8_t devAddress, ADS115Mux muxType, ADS115Pga pga, ADS115Mode mode, ADS1115DataRate dataRate, ADS1115CompMode compMode, ADS1115CompPol pol, ADS1115CompLatch latch, ADS1115CompQueue queue, TwoWire *wire = &Wire);
  ...
  private:
    uint8_t _deviceAddress = ADS1115_DEFAULT_ADDRESS;
    double _lsbSize;
