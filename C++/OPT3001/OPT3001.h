...
typedef enum 
{
  ...
  OPT_3001_AUTOMATIC_FULL_SCALE         = 0xC000  // Automatic scale
} Exponent_t;
...
class OPT3001{
  public:
    OPT3001(); // Constructor. It must be defined with the same name as the class

    void  init(uint8_t devAddress, TwoWire *wire = &Wire);
    void  init(uint8_t devAddress, Exponent_t exponentRange, ConversionTime_t conversionTime, ModeOfConversion_t mode, LatchField_t latchField, PolarityField_t polarity, MaskExponent_t mask, FaultCountField_t faultCount, TwoWire *wire = &Wire);
    
    void     writeRegister(uint8_t registerAddress, uint16_t value);
  ...
  private:
    uint8_t _deviceAddress = OPT3001_DEFAULT_ADDRESS;
