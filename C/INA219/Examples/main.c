...
static uint16_t calibration = 0;

static double rShunt        = 0.1;
...
/* USER CODE BEGIN PFP */
static void printValue(double registerValue, char mystring[]);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  while (1)
  {
	 dataReady = ina219DataReady(&ina219);
	 //printValue(dataReady, "Is bit ready: %d \r\n");
	 if (dataReady)
	 {
	  ...
	  if ((current - tickstart) >= 1000)
	  {
		  tickstart = current;
		  ina219DataReady(&ina219);
	   }
