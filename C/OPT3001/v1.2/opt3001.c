...
{
	CLEAR_BIT(opt3001->CONFIG, OPT3001_CR_RN);
	SET_BIT(opt3001->CONFIG, range);
	OPT3001_WriteRegister(opt3001, OPT3001_CONFIGURATION_REG, opt3001->CONFIG);
}
...
{
	CLEAR_BIT(opt3001->CONFIG, OPT3001_CR_M);
	SET_BIT(opt3001->CONFIG, convTime);
	OPT3001_WriteRegister(opt3001, OPT3001_CONFIGURATION_REG, opt3001->CONFIG);
}
...
void OPT3001_SetCT_800_MS(OPT3001_HandleTypeDef *opt3001)
{
	OPT3001_WriteRegister(opt3001, OPT3001_CONFIGURATION_REG, SET_BIT(opt3001->CONFIG, OPT3001_CR_CT));
}

void OPT3001_SetCT_100_MS(OPT3001_HandleTypeDef *opt3001)
{
	OPT3001_WriteRegister(opt3001, OPT3001_CONFIGURATION_REG, CLEAR_BIT(opt3001->CONFIG, OPT3001_CR_CT));
}
...
void OPT3001_Init(OPT3001_HandleTypeDef *opt3001, I2C_HandleTypeDef *i2c)
{
	...
	opt3001->hi2c = i2c;
	opt3001->CONFIG = OPT3001_GetConfiguration(opt3001);

	OPT3001_SetRangeNumber(opt3001, OPT3001_AUTOMATIC_RANGE);
	OPT3001_SetCT_100_MS(opt3001),
